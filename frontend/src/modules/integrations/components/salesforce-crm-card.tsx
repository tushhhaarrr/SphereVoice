...
interface SalesforceCrmCardProps {
...
export function SalesforceCrmCard({ integration, tenantId }: SalesforceCrmCardProps) {
  const [confirmDisconnect, setConfirmDisconnect] = useState(false);

  const initiate = useInitiateSalesforceOAuth();
      ...
      <Card className="w-full">
        <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-3">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-md bg-[#00a1e0] text-white font-bold text-sm select-none">
              SF
            </div>
            <div>
              <CardTitle className="text-base">Salesforce</CardTitle>
              <CardDescription className="text-xs">
                Sync contacts, leads & opportunities with your voice agents
              </CardDescription>
            </div>
          </div>
          ...
          {integration && (isConnected || isError) && (
            <div className="rounded-md border bg-muted/30 px-3 py-2 text-sm space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span>{integration.data_center}</span>
              </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              Salesforce data will not be deleted. You can reconnect at any time.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
